...
def find_products():
    ...
    for i in range(20):
        for j in range(20):
            if i <= 20 - 4:
                # Find product in the down direction
                res.append(arr[i][j] * arr[i+1][j] *
                           arr[i+2][j] * arr[i+3][j])
            if j <= 20 - 4:
                # Find product in the right direction
                res.append(arr[i][j] * arr[i][j+1] *
                           arr[i][j+2] * arr[i][j+3])
            if i <= 20 - 4 and j <= 20 - 4:
                # Find product in the down right direction
                res.append(arr[i][j] * arr[i+1][j+1] *
                           arr[i+2][j+2] * arr[i+3][j+3])
            if i <= 20 - 4 and j >= 3:
                # Find product in the down left direction
                res.append(arr[i][j] * arr[i+1][j-1] *
                           arr[i+2][j-2] * arr[i+3][j-3])
    return res


sol = max(find_products())
print(sol)
